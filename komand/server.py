...
class PluginServer(gunicorn.app.base.BaseApplication):
    ...
    def __init__(self, plugin, port=10001, workers=1, threads=4, debug=False):
        ...
        self.app = falcon.API()
        self.app.add_route("/{prefix}/{name}", self)
        self.app.add_route("/{prefix}/{name}/{test}", self)
    ...
    def on_post(self, req, resp, prefix, name, test):
        """
        Handles a POST request
        :param req: Incoming falcon.Request
        :param resp: Outgoing falcon.Response (to be served)
        :param prefix: Route prefix. Either "actions" or "triggers"
        :param name: Action/trigger name
        :param test: Route component indicating whether or not a test is occurring. "test" if test, None if normal
        :return: No return. Falcon will handle everything - just set properties on resp.
        """
        input_message = json.load(req.stream)
