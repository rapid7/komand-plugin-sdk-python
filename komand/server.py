...
from komand.api.schemas import PluginInfoSchema, ActionTriggerOutputBodySchema, ActionTriggerOutputSchema, \
    ActionTriggerInputBodySchema, ActionTriggerInputSchema, ActionTriggerDetailsSchema, ConnectionDetailsSchema, \
...
class PluginServer(gunicorn.app.base.BaseApplication):
    ...
    def create_flask_app(self):
        app = Flask(__name__)
        endpoints = Endpoints(self.logger, self.plugin, self.spec, self.debug, self.workers, self.threads, os.getpid())
    ...
    def register_blueprint(self):
        ...
        for blueprint in self.blueprints:
            self.app.register_blueprint(blueprint, url_prefix=VERSION_MAPPING[blueprint.name])
